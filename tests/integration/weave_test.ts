...
function hasMeshNamedNodeFact(
  quads: readonly Quad[],
  subjectPath: string,
  predicateIri: string,
  objectPath: string,
): boolean {
  const subjectIri = new URL(subjectPath, MESH_ALICE_BIO_BASE).href;
  const objectIri = new URL(objectPath, MESH_ALICE_BIO_BASE).href;
...
const secondAliceBioDefaultManifestation: readonly (readonly [
  string,
  string,
])[] = [[
...
const aliceReferenceDefaultManifestation: readonly (readonly [
  string,
  string,
])[] = [[
...
Deno.test("executeWeave materializes current support ResourcePages for a docs-rooted sidecar mesh", async () => {
  ...
  assertFalse(configPage.includes("sflo:ArtifactHistory"));
  assertFalse(configPage.includes("sflo:HistoricalState"));
  assertFalse(configPage.includes("sflo:ArtifactManifestation"));
  assertFalse(configPage.includes("sflo:LocatedFile"));
  assertStringIncludes(
    configPage,
...
Deno.test("executeWeave first-payload locator conflict writes no files and preserves MeshInventory bytes", async () => {
  const workspaceRoot = await createTestTmpDir(
    "weave-weave-first-payload-conflict-",
  );
  ...
  const conflictingInventory = currentInventory.replace(
    ...
    "sflo:hasWorkingLocatedFile <other-alice-data.ttl> .",
  );
  ...
  await assertRejects(
    () =>
      executeWeave({
        ...
        request: { targets: [{ designatorPath: "alice/data" }] },
      }),
    Error,
...
Deno.test("executeWeave rejects legacy first-payload inventory progression without writes", async () => {
  const workspaceRoot = await createTestTmpDir(
    "weave-weave-first-payload-legacy-progression-",
  );
  await materializeMeshAliceBioBranch(
    "06-alice-bio-integrated",
    workspaceRoot,
...
Deno.test("executeWeave supports the exact root target", async () => {
  ...
  const meshInventory = await Deno.readTextFile(
    join(workspaceRoot, "_mesh/_inventory/inventory.ttl"),
  );
  const meshInventoryQuads = new Parser({
    baseIRI: MESH_ALICE_BIO_BASE,
  }).parse(meshInventory) as Quad[];
  assert(
    hasMeshNamedNodeFact(
      meshInventoryQuads,
      "_mesh",
...
Deno.test("executeWeave matches the settled alice bio integrated-woven fixture", async () => {
  ...
  assert(result.updatedPaths.includes("_mesh/_inventory/inventory.ttl"));
  assert(
    result.updatedPaths.includes("alice/data/_knop/_inventory/inventory.ttl"),
  );
  assertEquals(
    await Deno.readTextFile(
      join(workspaceRoot, "_mesh/_inventory/inventory.ttl"),
    ...
    replaceFixturePaths(
      ...
      firstAliceBioDefaultManifestation,
    ),
  );
  assertEquals(
    await Deno.readTextFile(
  ...
  await Deno.stat(join(workspaceRoot, "alice/data/index.html"));
  const knopPage = await Deno.readTextFile(
    join(workspaceRoot, "alice/data/_knop/index.html"),
  );
  assertStringIncludes(knopPage, "<h2>Governed Artifacts</h2>");
  assertStringIncludes(knopPage, "PayloadArtifact");
  assertStringIncludes(knopPage, 'href="/mesh-alice-bio/alice/data"');
  assertStringIncludes(knopPage, "<h2>Supporting Artifacts</h2>");
  assertStringIncludes(
    knopPage,
    'href="/mesh-alice-bio/alice/data/_knop/_meta"',
  );
  assertStringIncludes(
    knopPage,
    'href="/mesh-alice-bio/alice/data/_knop/_inventory"',
  );
...
`,
  ...
  assertEquals(result.wovenDesignatorPaths, ["alice", "alice/data"]);
  const meshInventory = await Deno.readTextFile(
    join(workspaceRoot, "_mesh/_inventory/inventory.ttl"),
  );
  const meshInventoryQuads = new Parser({
...
`,
  ...
  await assertRejects(
    () => Deno.stat(join(workspaceRoot, "alice/data/_history001")),
...
Deno.test("executeWeave can start a named payload history after an ordinal history exists", async () => {
  ...
  assertStringIncludes(
    inventory,
    "<alice/data/releases> a sflo:ArtifactHistory ;",
  );
  assertStringIncludes(
    inventory,
    "sflo:hasHistoricalState <alice/data/releases/v0.0.1> ;",
  );
  assertStringIncludes(
    inventory,
  ...
  assert(
    !inventory.includes(
      "<alice/data/releases/v0.0.1> a sflo:HistoricalState ;\n  sflo:stateOrdinal",
    ),
  );
});
...
Deno.test("executeWeave forwards targets to generate and refreshes ancestor pages", async () => {
  ...
  assertStringIncludes(
    alicePage,
    '<nobr><a class="wf-child-identifier" href="/mesh-alice-bio/alice/data">data</a></nobr>',
  );
  await Deno.stat(join(workspaceRoot, "alice/data/index.html"));
});
...
Deno.test("executeWeave resolves payload current files from workingLocalRelativePath literals", async () => {
  ...
  await replaceFileText(
    join(workspaceRoot, "alice/data/_knop/_inventory/inventory.ttl"),
    `sflo:hasWorkingLocatedFile <alice-data.ttl> ;`,
    `sflo:workingLocalRelativePath "alice-data.ttl" ;`,
  );
  ...
  assertStringIncludes(
    ...
    `/mesh-alice-bio/alice/data/_history001/_s0002/ttl/alice-data.ttl`,
  );
});
...
`,
  ...
  const sharedPayloadPath = join(repoRoot, "documentation/alice-data.ttl");
  await Deno.writeTextFile(
    sharedPayloadPath,
    await Deno.readTextFile(join(workspaceRoot, "alice-data.ttl")),
  );
...
Deno.test("executeWeave materializes the second alice bio payload weave slice", async () => {
  ...
  assertEquals(
    ...
    replaceFixturePaths(
      ...
      secondAliceBioDefaultManifestation,
    ),
  );
  assertEquals(
    await Deno.readTextFile(
