...
- [ ] Generalize the support-history policy seam beyond mesh support ResourcePage catch-up.
- [ ] Classify at least `_mesh/_meta`, `_mesh/_config`, `_mesh/_knop-inheritable-config`, `_knop/_meta`, `_mesh/_inventory`, `_knop/_inventory`, payload artifacts, `ResourcePageDefinition`, and `ReferenceCatalog`.
...
- [ ] Keep `_mesh/_config` versioned in mesh-support page planning unless an explicit future policy overrides it.
