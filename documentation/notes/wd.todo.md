...
- [ ] Publish Weave's programmatic surface as a consumable library (JSR export and/or npm library package) per [[wd.library-packaging]], with a resource-loading audit (the accord JSR shapes-loading bug class), a downstream off-tree contract smoke test, and release-runbook/CI wiring. `v0.4.0` shipped the API as a source-only import; this is the deferred packaging follow-up. Active on `next/v0.4.1`.
