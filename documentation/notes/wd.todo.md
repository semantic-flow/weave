...
- [ ] Run the planning loop from [[wa.task.2026.2026-07-31_1014-planning-loop-infrastructure]]: the queue gate (`deno task queue`), [[wd.queues]], [[wd.read-in.jimbo]], and [[wa.dave-court]] are merged to main (PR #30) with all decisions ruled; remaining is the dry run (one supervised `/loop 10m` wake with Dave).
