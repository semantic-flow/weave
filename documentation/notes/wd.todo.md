...
- [x] Publish Weave's programmatic surface as a consumable library per [[wd.library-packaging]]: done in `v0.5.0` as the npm package `@semantic-flow/weave-lib` (dnt build, resource-loading audit + embedded defaults, fs-purity guard, off-tree Node contract smoke, release-runbook/CI wiring). JSR export remains deferred until the npm path is proven with a downstream consumer.
