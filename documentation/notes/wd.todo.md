...
- [x] Release `v0.7.0`: **RELEASED 2026-08-07** (tag `v0.7.0`, npm `@semantic-flow/weave@0.7.0` and `@semantic-flow/weave-lib@0.7.0`, GitHub Release marked Latest; run 31227190212, all 13 jobs green). Carries PRs #31/#33/#34/#35 plus ESM-only packaging (#39). First release able to generate the published SFLO mesh — `v0.6.0` provably cannot. Minor rather than patch because the untargeted batch dispatch makes a previously-refused invocation succeed; also breaking for CommonJS consumers of `weave-lib`, whose published `exports` is now `{".": {"import": "./esm/api/mod.js"}}` with no `require` entry. Was blocked 2026-08-02 → 08-07 by a GitHub Actions outage, not by anything in the repo.
