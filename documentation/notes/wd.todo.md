...
- [ ] Fix later binary payload advancement as a dedicated end-to-end byte-preservation task. The evidence in [[wa.task.2026.2026-07-21_1322-programmatic-version-api]] r1 F3 shows that later advancement currently writes decoded `currentPayloadTurtle` through a text `PlannedFile` even though the loader retains `currentPayloadBytes`; own working update, first/later history advancement, no-op comparison, and exact-byte tests together before admitting binary content to `versionPayloads`.
- [ ] Add a real library distribution vehicle for the stable programmatic API from [[wd.programmatic-version-api]]: define npm and/or JSR exports, package the root API and declarations, and add downstream import/contract smoke tests. Until then, v1 consumers use `./src/mod.ts` from a full-commit-pinned Weave checkout, per the PM GO ruling.
- [ ] Make extracted-term weave viable for thousand-term nested-source meshes, following the evidence in [[wa.task.2026.2026-07-21_1603-extractor-defect-pair]]: remove `planFirstExtractedKnopWeave`'s assumption that the source payload's top-level designator already has a Knop, add a bounded-memory batch path that avoids repeatedly parsing and rewriting the whole MeshInventory under the roughly 4 GiB V8 heap ceiling, make targeted and untargeted extract→weave→generate agree, render MeshInventory history indexes from actual progression instead of the fixed `_s0001`–`_s0004` list, and add a roughly 1,700-term regression workload.
...
- [x] Publish Weave's programmatic surface as a consumable library per [[wd.library-packaging]]: shipped in `v0.5.1` as the npm package `@semantic-flow/weave-lib` (dnt build, resource-loading audit + embedded defaults, fs-purity guard, off-tree Node contract smoke, release-runbook/CI wiring; the planned `v0.5.0` was folded into `v0.5.1` and the lib-only `weave-lib@0.5.0` artifact deprecated). JSR export remains deferred until the npm path is proven with a downstream consumer.
- [ ] Add a programmatic `validateMesh(meshRoot, options)` export to `weave-lib` returning structured findings (path, severity, code, message) instead of formatted text — the adoption-deciding ask from the 2026-07-28 Stagecraft consumer review ([[wd.consumer-feedback-0.5.1]] §8, deferred from v0.5.1). Needs a designed findings contract with the same rigor `versionPayloads` got; candidate headline for v0.6.0.
