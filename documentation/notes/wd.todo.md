...
- [ ] Remove the remaining `ResourcePageDefinition` history dependency: `_knop/_page` should be able to stay current-only when history policy says current-only, and ResourcePage generation should make a best-effort current rendering when `_knop/_page/_history001` is absent. `config` remains the explicit versioned support-artifact exception.
