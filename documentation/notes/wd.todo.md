...
- [ ] Release `v0.7.0`: STAGED at `7915eae`, BLOCKED on a GitHub Actions `major_outage` as of 2026-08-06 — not on us. Five user-facing changes since `v0.6.0` (PRs #31/#33/#34/#35 plus ESM-only packaging in #39), and the first release able to generate the published SFLO mesh. Minor rather than patch because the untargeted batch dispatch makes a previously-refused invocation succeed; also breaking for CommonJS consumers of `weave-lib`. Remaining: rehearse on the merge commit, inspect, publish. No tag yet; npm still serves 0.6.0.
