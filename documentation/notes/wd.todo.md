...
- [ ] Close out [[wa.task.2026.2026-07-03_1332-stagecraft-weave-planner-generalization]]: single- and multi-target later-payload advancement landed earlier, and condition-specific planner diagnostics landed post-regroom (PR #42, plus the approved malformed-knop-metadata finding code `ec1f35d`). Audit the note against `main` and either close it or narrow it to residuals it still owns.
...
4. **Append-only inventory migration** — [[wa.task.2026.2026-05-17-append-onlyish-inventory]]: planner and first consumer shipped in `v0.7.0`; migrating the remaining writers is incremental correctness work that can ride alongside any arc.
...
- [[wa.task.2026.2026-07-03_1332-stagecraft-weave-planner-generalization]]: largely landed — later-payload advancement (single and multi target) plus condition-specific diagnostics (PR #42). Audit against `main`, then close or narrow.
