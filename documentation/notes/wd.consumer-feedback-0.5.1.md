...
- **Prerequisite, not assumption (Codex):** the v0.5.0 release must be completed first — PR #22 merged, Release Manual run (wrapper/platform publish + GitHub Release + `v0.5.0` tag), and the `weave-lib` trusted publisher configured. v0.5.1 is the first release expected to publish `weave-lib` through OIDC end to end.
...
- [ ] Release notes v0.5.1 (this slice + retroactive 4 MiB callout naming `23f50af` shipped in v0.4.0) and version bump, after the v0.5.0 release completes.
...
- The v0.5.0 release (PR #22 merge, Release Manual run, trusted-publisher configuration) must complete before v0.5.1 can ship; implementation can proceed on `next/v0.5.1` meanwhile.
