...
- `src/core/weave/mesh_inventory_renderers.ts` builds only the term, Knop, page, and optional history/state facts owned by extracted-term weave, then appends them against the original MeshInventory. Homogeneous batches and the versioned sequential path no longer filter, replace, or reconstruct target subject blocks; they preserve carried bytes and graph while retaining canonical batch order and the existing progression cardinality.
