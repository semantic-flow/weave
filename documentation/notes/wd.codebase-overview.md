...
- `src/core/weave/mesh_inventory_renderers.ts` builds only the term, Knop, page, membership, and optional history/state facts owned by each migrated path, then appends them against the original MeshInventory. Versioned first-Knop weave, homogeneous extracted batches, versioned sequential extracted weave, and the shared current-only first-payload/extracted helper preserve carried bytes and graph. First-Knop weave rejects legacy inventory-owned mutable progression before planning instead of silently deleting or retaining it; current-only semantic no-ops return exact input bytes and create no history progression.
...
- The Alice, sidecar, and branch-published live fixture tails were regenerated and published on 2026-08-31 after mesh-support progression moved to MeshMetadata. Alice `main` matches `a.30-founding-corrected`, sidecar `main` matches `a.17-all-remaining-terms-woven`, and branch publication `gh-pages` matches `a.15-extracted-term-references-woven`.
