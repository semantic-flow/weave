...
- `src/core/weave/mesh_inventory_renderers.ts` builds only the term, Knop, page, and optional history/state facts owned by a homogeneous extracted-term batch, then appends them against the original MeshInventory. That batch path no longer filters or reconstructs target subject blocks; current-only and versioned policies preserve the carried bytes and graph while retaining canonical target order and one shared progression.
