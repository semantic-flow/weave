...
- `src/runtime/atomic_file_plan.ts` provides preflight plus in-process rollback for Knop creation and composed founding correction. `src/runtime/knop/founding_validation.ts` checks immutable snapshot digests and publication settlement without network access.
