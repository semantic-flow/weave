...
- `src/core/knop/create.ts` preserves the complete carried MeshInventory for first and later Knop creation. `create_inventory_index.ts` builds the operation-scoped membership/type/slot index from the planner's prepared quads, avoiding one full quad scan per existing Knop while retaining fail-closed shape checks.
