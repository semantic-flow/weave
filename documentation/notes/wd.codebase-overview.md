...
### core
  ...
  `core/targeting.ts` owns portable target request semantics: validate/generate target normalization, version-target normalization, prepared weave target derivation for version plus shared phases, root/recursive selection, and requested-target coverage rules including the intentional exact-versus-recursive asymmetry
  ...
  `core/weave` has started splitting focused planners and shared models out of the large façade module; mesh support ResourcePage catch-up planning now lives in `mesh_support_pages.ts`, shared ResourcePage model types live in `resource_page_models.ts`, pure ResourcePage history grouping lives in `resource_page_history_groups.ts`, pure ReferenceCatalog page-link extraction lives in `resource_page_reference_links.ts`, while `weave.ts` keeps the public re-export surface for existing runtime, CLI, and test imports
...
### runtime
  ...
  runtime weave orchestration prepares a `PreparedWeaveExecution` once from shared target semantics, then dispatches version planning/writes and page generation from that prepared object instead of re-deriving generate targets from the raw request
  `runtime/weave/weave.ts` remains the public façade for validate/version/generate/weave operations, while `runtime/weave/errors.ts`, `runtime/weave/mesh_state.ts`, `runtime/weave/planning_context.ts`, `runtime/weave/artifact_loaders.ts`, `runtime/weave/candidate_loader.ts`, `runtime/weave/request_normalization.ts`, `runtime/weave/prepared_execution.ts`, `runtime/weave/version_execution.ts`, `runtime/weave/execution_config.ts`, `runtime/weave/workspace_paths.ts`, `runtime/weave/progress.ts`, `runtime/weave/page_generation.ts`, `runtime/weave/page_contexts.ts`, `runtime/weave/page_model_assembly.ts`, `runtime/weave/raw_source_panels.ts`, and `runtime/weave/timing_helpers.ts` now hold shared runtime errors, mesh-state loading, command-scoped staged filesystem/cache behavior, artifact-loading helpers used by both candidate loading and page context loading, candidate discovery/coverage checks, request normalization, prepared execution, version planning/writes, effective config policy adaptation, workspace path conversion, progress types, ResourcePage generation orchestration and generated-page writes, ResourcePage designator-context loading, ResourcePage model assembly, ResourcePage raw-source panel loading, and optional timing wrappers. The planning context owns the overlay-aware read cache, dependency-aware candidate cache, planned-file staging, and cache counters used by `WEAVE_TIMING`.
