...
- Already extracted core weave helpers include `errors.ts`, `version_plan.ts`, `mesh_support_pages.ts`, `requests.ts`, `source_models.ts`, `candidates.ts`, `planning_models.ts`, `slices.ts`, `resource_page_models.ts`, `resource_page_history_groups.ts`, `resource_page_policy.ts`, `resource_page_reference_links.ts`, `naming_policy.ts`, and `support_history_policy.ts`.
...
- First core weave extraction slice, now partly stale because ResourcePage models moved during runtime page-generation work: [[wa.task.2026.2026-05-21_1037-core-weave-first-extraction-slice]].
