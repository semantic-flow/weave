...
- `src/core/weave/mesh_inventory_renderers.ts` builds only the term, Knop, page, membership, and optional history/state facts owned by each migrated path, then appends them against the original MeshInventory. Homogeneous extracted batches, versioned sequential extracted weave, and the shared current-only first-payload/extracted helper preserve carried bytes and graph; current-only semantic no-ops return exact input bytes and create no history progression.
- `src/core/weave/mesh_support_pages.ts` keeps initial support-history construction separate, but its no-initial-history page-only arm prepares the carried MeshInventory and appends only named support-page claims/types. Exact duplicates produce no inventory update; comments, repeated blocks, opaque facts, trailing bytes, and carried blank nodes remain untouched prefix data.
...
- `src/core/knop/create.ts` preserves the complete carried MeshInventory for first and later Knop creation and optionally plans exact binary founding bytes plus append-planned KnopInventory discovery facts. `founding_referent_data.ts` enforces the bounded flat Turtle profile; `version_founding_referent_data.ts` plans no-page immutable founding states through the shared inventory append primitive. `create_inventory_index.ts` builds the operation-scoped membership/type/slot index from prepared quads.
- `src/runtime/atomic_file_plan.ts` provides preflight plus in-process rollback for Knop creation and composed founding correction, tracking operation-created directories so pre-existing empty ancestors survive rollback. `src/runtime/knop/founding_validation.ts` checks immutable snapshot digests and publication settlement without network access and fails closed when a registered Knop inventory is missing or unparseable.
...
- Full generation visits page models in canonical order, renders and writes at most four pages per batch, and discards each batch before continuing. `raw_source_panels.ts` shares decoded source contents by absolute path for the command so many extracted pages can inspect one source without retaining one full source string per page. See [[wa.completed.2026.2026-08-21_1111-generate-streaming-memory]].
...
- The Alice `a.04` and sidecar `a.10` fixture-branch MeshInventory files still reflect the retired block-replacement output that dropped carried config facts. Current create E2E coverage compares exact RDF against the stale expected graph plus the explicitly carried `_mesh/_config` graph; regenerate those ladder rungs when the broader fixture update is scheduled.
