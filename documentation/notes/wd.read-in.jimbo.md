...
- **Append-only inventory migration is active** — add-reference, extracted/current-only MeshInventory, current-only PageDefinition, and current-shape extract are delivered under [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]. [[wa.task.2026.2026-08-31_1219-mesh-support-page-only-inventory-append]] is next. [[wa.dave-court]] has two plan-owned cards: named repository-locator identity and legacy MeshInventory progression migration.
