...
- The two Codex seats: adversarial spec reviewer of task notes (read-only), and `codex exec` implementation on `lane/*` branches. Invocation gotchas: from non-TTY shells run `codex exec … < /dev/null` (it hangs reading stdin otherwise), and the workspace-write sandbox mounts `.git` read-only — Jimbo creates the lane branch before firing Kim and owns the commit after her edits (credit the receipts to "Codex (codex exec)").
...
- The planning loop is LIVE and its infrastructure task is CLOSED (renamed to [[wa.completed.2026.2026-07-31_1014-planning-loop-infrastructure]] at wake 2 after the 2026-08-01 dry run: merged PR #30, all D1–D8 ruled). Extractor bite 1 (nested sources without root Knops) LANDED on PR #31 — merge GO with Dave (court card); remaining extractor slices keep the queue entry. The Stagecraft requirements collection is folded into its note (eleven cited requirements + ownership map); only the evidence-gated topology decision keeps that task open. Known gap: the binary-payload wd.todo item still needs its task note cut before it can queue.
