...
- Groom duties (once-per-day floors, stamped mechanically per D8): `read-in` (this note's currency, especially § The active arc), `queues` (`deno task queue check`), `court` (open cards only; sweep ruled ones), `todo` ([[wd.todo]] sync), `closure` (task-note closure flags — landed-but-un-renamed notes), `decision-log` ([[wd.decision-log]] currency). `deno task queue wake` reports unmet floors; `deno task queue groomed <duty>` stamps one.
...
- v0.6.0 RELEASED 2026-07-31: programmatic `validateMesh` v1 per the ratified contract [[wd.programmatic-validate-api]], plus the whole-mesh validate bounded-memory fix (srd-scale OOM → ~554 MiB). Receipts live on the two 07-29 archive task notes, which still await Dave's rename to `wa.completed.*`.
