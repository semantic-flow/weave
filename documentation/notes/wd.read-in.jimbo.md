...
- **Append-only inventory migration is active** — add-reference, extracted/current-only MeshInventory, and current-only PageDefinition are delivered under [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]. [[wa.task.2026.2026-08-31_1147-current-shape-extract-mesh-inventory-append]] is next. [[wa.dave-court]] has two plan-owned cards: named repository-locator identity and legacy MeshInventory progression migration.
