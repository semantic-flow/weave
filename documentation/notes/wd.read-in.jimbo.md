...
- The two Codex seats: adversarial spec reviewer of task notes (read-only), and `codex exec` implementation on `lane/*` branches. Invocation gotcha: from non-TTY shells run `codex exec … < /dev/null` — it hangs reading stdin otherwise.
...
- The planning-loop infrastructure MERGED to main 2026-08-01 (PR #30) with every decision D1–D8 ruled per [[wa.task.2026.2026-07-31_1014-planning-loop-infrastructure]]; the supervised dry run started the same day (loop armed at 10m; first wake fired the extractor bite-carving analysis via codex read-only).
