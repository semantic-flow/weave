...
- **v0.7.0 RELEASED 2026-08-07** — tag `v0.7.0`, both npm packages at 0.7.0, GitHub Release Latest. `weave-lib` is now ESM-only (`exports` has no `require` entry), which is breaking for CommonJS consumers. Deliberately shipped WITHOUT the extracted-term batch path (#41, held on a confirmed stale-history-index bug) and the planner diagnostics (#42, consumer-visible error-string changes with no soak, plus an open finding-code question). Both target v0.8.0.
...
- **First staleness sweep of the back catalogue ran 2026-08-02** (PR #37, archive `48d6657`): all 41 open `wa.task.*` classified against code — 7 closed, 4 cancelled under the new `wa.cancelled.*` prefix, 27 live, 3 unclear. [[wd.todo]] regroomed against its CONFLICTS section. The extractor defect pair CLOSED only after its scale epic was carved into [[wa.task.2026.2026-08-02_1330-extracted-term-weave-batch-path]], now queue item 1.
