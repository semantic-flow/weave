...
The planning session (Jimbo) runs on a recurring `/loop`. [[wd.read-in.jimbo]] is the seating source and carries the canonical loop prompt; [[wd.queues]] is the gated READY slice, written only through `deno task queue` (add/pop/check/wake/groomed — run `check` before any commit touching it); open decisions for Dave live in [[wa.dave-court]] (archive vault, ungated). The law lives in those notes and in `wa.task.2026.2026-07-31_1014-planning-loop-infrastructure`; this paragraph is only the pointer.
