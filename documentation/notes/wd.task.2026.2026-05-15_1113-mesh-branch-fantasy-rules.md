...
- How should local preview worktrees that check out `gh-pages` be handled during reruns? They are useful for inspection, but they can block moving the local publication branch to the latest accepted numeric checkpoint.
...
- [ ] Fast-forward the publication branch, probably `gh-pages`, to the latest generated publication rung after review; keep `main` clean source. After the `_sources` rerun, preview worktrees still hold `gh-pages`, so this needs an intentional cleanup/update step.
