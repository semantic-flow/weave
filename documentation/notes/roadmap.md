...
4. **Append-only inventory migration** — [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]: both blocking rulings are settled; next cut named locator implementation and progression producer/fixture correction before resuming first-Knop append.
...
- `[importance: medium] [how-soon: next]` Replace remaining inventory rewrites with append-onlyish writes. Both rulings are settled: named locators use `_sources#payload-source-repository-locator`, and first-Knop append follows producer/fixture correction then fail-closed stale-shape handling.
