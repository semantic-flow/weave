...
- `[importance: high] [how-soon: next]` Introduce a knop-owned `ResourcePageDefinition`-style support artifact at `_knop/_page` so identifiers such as `alice/` can have a customized public page without pretending the identifier itself is a payload-bearing DigitalArtifact. Keep control metadata local while allowing page content to come from mesh-local `targetMeshPath` values, in-mesh artifacts, or imported outside content. See [[wd.task.2026.2026-04-08_1545-resource-page-definition-and-sources]].
- `[importance: high] [how-soon: next]` Keep ordinary Markdown as the default authored content format for mesh-local page files referenced from `_knop/_page/page.ttl`, and if Dendron-style frontmatter, wikilinks, or naming conventions are later supported, model them as an optional source-interpretation profile rather than the base page-definition contract. See [[wd.task.2026.2026-04-08_1545-resource-page-definition-and-sources]].
...
- `[importance: high] [how-soon: next]` Let `_knop/_page/page.ttl` point directly at mesh-local `targetMeshPath` values and in-mesh DigitalArtifacts, but require outside-the-tree or extra-mesh content to cross an explicit import boundary first so the imported in-tree copy becomes the governed artifact and current `WorkingLocatedFile` that page generation follows. See [[wd.task.2026.2026-04-08_1545-resource-page-definition-and-sources]].
- `[importance: high] [how-soon: next]` Add host/operational config support for allowed local directories so extra-mesh `workingFilePath` and `targetMeshPath` values may use `../` outside the mesh root without persisting absolute host paths in core RDF. Keep this boundary policy operational, and use the old `sflo-host` line only as precedent rather than as a drop-in answer. See [[wd.task.2026.2026-04-08_1545-resource-page-definition-and-sources]] and [[wd.task.2026.2026-04-08_1735-page-definition-ontology-and-config]].
...
- `[importance: medium] [how-soon: next]` Continue the `mesh-alice-bio` fixture ladder with root-focused transitions when root lifecycle coverage is ready. Adding a root later in the mesh lifecycle is a legitimate case, so root behavior should stay on the carried ladder unless later evidence shows that split fixtures are clearer.
