...
4. **Append-only inventory migration** — [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]: the independent current-shape extract and mesh-support page-only children have landed; remaining evident writers wait on progression or the exact IRI for the ruled named repository locator.
...
- `[importance: medium] [how-soon: later]` Replace remaining inventory rewrites with append-onlyish writes. [[wa.completed.2026.2026-08-31_1219-mesh-support-page-only-inventory-append]] closed the last evident independent slice; progression migration and the exact named-locator IRI now wait in [[wa.dave-court]].
