...
4. **Append-only inventory migration** — [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]: migrate remaining writers through reviewable children; current-shape extract has landed and mesh-support page-only growth is next while two versioned-writer rulings are open.
...
- `[importance: medium] [how-soon: next]` Replace remaining inventory rewrites with append-onlyish writes. See [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]; [[wa.task.2026.2026-08-31_1219-mesh-support-page-only-inventory-append]] is next while first-Knop progression migration and first-payload locator identity wait in [[wa.dave-court]].
