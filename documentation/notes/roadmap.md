...
4. **Append-only inventory migration** — [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]: migrate the remaining inventory writers through independently reviewable children; both extracted versioned paths are complete, and the shared current-only first-payload/extracted helper is next.
...
- `[importance: medium] [how-soon: next]` Replace remaining inventory rewrites with append-onlyish writes. See [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]; the shared planner and several consumers have shipped, and [[wa.task.2026.2026-08-31_1050-current-only-payload-like-mesh-inventory-append]] is the first remaining child.
