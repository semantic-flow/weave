...
4. **Append-only inventory migration** — [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]: migrate remaining writers through reviewable children; extracted/current-only paths are complete, versioned first-Knop is next, and repository-locator identity gates versioned first-payload.
...
- `[importance: medium] [how-soon: next]` Replace remaining inventory rewrites with append-onlyish writes. See [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]; [[wa.task.2026.2026-08-31_1111-versioned-first-knop-mesh-inventory-append]] is next while repository floating-locator identity waits in [[wa.dave-court]].
