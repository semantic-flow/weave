...
4. **Append-only inventory migration** — [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]: migrate remaining writers through reviewable children; current-only PageDefinition is next while two MeshInventory contract rulings are open.
...
- `[importance: medium] [how-soon: next]` Replace remaining inventory rewrites with append-onlyish writes. See [[wa.plan.2026.2026-05-17-append-onlyish-inventory]]; [[wa.task.2026.2026-08-31_1127-current-only-page-definition-inventory-append]] is next while first-Knop progression migration and first-payload locator identity wait in [[wa.dave-court]].
